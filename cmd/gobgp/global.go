...
func lsParseSegmentListArg(arg string) (*bgp.LsSrSegmentList, error) {
	...
	sl := &bgp.LsSrSegmentList{
		Flags:  bgp.LsSrSegmentListFlags{Explicit: true, Computed: true},
		Weight: uint32(weight),
	}
	for _, sidStr := range strings.Split(sidsStr, ",") {
		seg := bgp.LsSrSegment{Flags: bgp.LsSrSegmentFlags{SIDPresent: true, Explicit: true}}
...
func lsParseSrPolicyConstraintsArgs(m map[string][]string) (*bgp.LsSrCandidatePathConstraints, error) {
	...
	for _, key := range []string{
		"constraint-flags", "constraint-mtid", "constraint-algorithm",
		"constraint-exclude-any", "constraint-include-any", "constraint-include-all",
		"constraint-srlg", "constraint-bandwidth", "constraint-disjoint-group",
		"constraint-bidir-group", "constraint-metric",
	} {
		if len(m[key]) > 0 {
			present = true
			break
...
func parseLsSrPolicyCandidatePathNLRIType(args []string) (bgp.NLRI, *bgp.PathAttributeLs, error) {
	...
	if hasPriority || hasPreference || hasStateFlags {
		...
		if hasStateFlags && len(stateFlags) > 0 {
			for _, r := range strings.ToUpper(stateFlags[0]) {
				switch r {
				case 'S':
				...
				case 'B':
					state.Flags.Backup = true
				case 'E':
					state.Flags.Evaluated = true
				case 'V':
					state.Flags.ValidSIDList = true
				case 'O':
					state.Flags.OnDemand = true
				case 'D':
					state.Flags.Delegated = true
				case 'C':
					state.Flags.Provisioned = true
				case 'I':
					state.Flags.DropUponInvalid = true
				case 'T':
					state.Flags.TransitEligible = true
				case 'U':
					state.Flags.Dropping = true
				default:
					return nil, nil, fmt.Errorf("invalid state-flags %q: expected letters from SABEVODCITU", stateFlags[0])
				}
			}
		}
		sp.State = state

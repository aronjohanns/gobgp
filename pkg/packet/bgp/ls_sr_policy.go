...
// (256) and the SR Policy Candidate Path Descriptor TLV (554).
...
func (l *LsSrPolicyCandidatePathNLRI) String() string {
	...
	// LsNodeDescriptor.String omits the IGP descriptors when a BGP Router-ID
	// is present. All of them, including the headend address, are part of
	// this NLRI's key and must distinguish destinations in the RIB.
	headend := fmt.Sprintf("{ASN: %d, BGP LS ID: %d, OSPF AREA: %d, IGP ROUTER ID: %s, BGP ROUTER ID: %s, CONFEDERATION: %d, IPv4: %s, IPv6: %s}",
		local.Asn, local.BGPLsID, local.OspfAreaID, local.IGPRouterID, local.BGPRouterID,
		local.BGPConfederationMember, local.LocalRouterID, local.LocalRouterIDv6)
	return fmt.Sprintf("SRPOLICY_CP { LOCAL_NODE: %s ENDPOINT: %s COLOR: %d ORIGIN: %d ORIGINATOR: %d/%s DISCRIMINATOR: %d %s:%d }",
		headend, cp.Endpoint, cp.Color, cp.ProtocolOrigin, cp.OriginatorASN, cp.OriginatorAddress, cp.Discriminator,
		l.ProtocolID.String(), l.Identifier)
}
...
func (l *LsSrPolicyCandidatePathNLRI) DecodeFromBytes(data []byte) error {
	...
	for len(tlvs) >= tlvHdrLen {
		...
		// Unknown TLVs and repeated instances of a TLV (RFC 9857: the first
		// valid one is used) are skipped.
		if tlv == nil {
			tlvs = tlvs[hdr.Len():]
			l.Length -= uint16(hdr.Len())
			continue
		}
...
func (l *LsSrPolicyCandidatePathNLRI) Serialize() ([]byte, error) {
	...
	s, err = l.CandidatePathDesc.Serialize()
	if err != nil {
		return nil, err
	}
	buf = append(buf, s...)
...
func (l *LsTLVSrSegmentListBandwidth) DecodeFromBytes(data []byte) error {
	...
	if l.Bandwidth < 0 || math.IsNaN(float64(l.Bandwidth)) || math.IsInf(float64(l.Bandwidth), 0) {
		return malformedAttrListErr("Incorrect SR Segment List Bandwidth value")
	}
...
// Validate checks fields whose interpretation depends on the segment type.
// In particular, an IPv6 address in an IPv4 descriptor would panic in As4
// during serialization, after an API request has already been accepted.
func (s *LsSrSegment) Validate() error {
	if _, ok := lsSrSegmentSIDLen(s.SegmentType); !ok {
		return fmt.Errorf("unknown SR segment type %d", s.SegmentType)
	}
	if s.Label > 0xfffff {
		return fmt.Errorf("SR segment label exceeds 20 bits: %d", s.Label)
	}
	if s.SID.IsValid() && (!s.SegmentType.IsSRv6() || !s.SID.Is6() || s.SID.Is4In6() || s.SID.Zone() != "") {
	...
	check := func(addr netip.Addr, ipv6 bool, field string) error {
		if !addr.IsValid() || addr.Zone() != "" || addr.Is4In6() || addr.Is6() != ipv6 {
	...
	case LS_SR_SEGMENT_TYPE_D_IPV6_NODE_MPLS, LS_SR_SEGMENT_TYPE_G_IPV6_NODE_INTERFACE_MPLS,
		LS_SR_SEGMENT_TYPE_H_IPV6_ADJACENCY_MPLS, LS_SR_SEGMENT_TYPE_I_IPV6_NODE_SRV6,
		LS_SR_SEGMENT_TYPE_J_IPV6_NODE_INTERFACE_SRV6, LS_SR_SEGMENT_TYPE_K_IPV6_ADJACENCY_SRV6:
	...
	case LS_SR_SEGMENT_TYPE_G_IPV6_NODE_INTERFACE_MPLS, LS_SR_SEGMENT_TYPE_H_IPV6_ADJACENCY_MPLS,
		LS_SR_SEGMENT_TYPE_J_IPV6_NODE_INTERFACE_SRV6, LS_SR_SEGMENT_TYPE_K_IPV6_ADJACENCY_SRV6:
...
func (l *LsTLVSrSegment) Serialize() ([]byte, error) {
	s := l.Extract()
	// Validate the raw SID fields too, even when S is clear.
	s.SID, s.Label = l.SID, l.Label
...
func (l *LsTLVSrBandwidthConstraint) DecodeFromBytes(data []byte) error {
	...
	if l.Bandwidth < 0 || math.IsNaN(float64(l.Bandwidth)) || math.IsInf(float64(l.Bandwidth), 0) {
		return malformedAttrListErr("Incorrect SR Bandwidth Constraint value")
	}

...
const srPolicyHeadendStr = "{ASN: 65001, BGP LS ID: 0, OSPF AREA: 0, IGP ROUTER ID: , BGP ROUTER ID: 1.1.1.1, CONFEDERATION: 0, IPv4: 10.0.0.1, IPv6: invalid IP}"
...
func Test_PathAttributeLsSrPolicyMalformed(t *testing.T) {
	...
	tests := []struct {
		...
		{"bandwidth negative", lsAttrBytes(srPolicySegmentListTLV(0, 1, tlvBytes(1216, []byte{0xbf, 0x80, 0, 0})))},
		...
		{"bandwidth constraint negative", lsAttrBytes(srPolicyConstraintsTLVWith(tlvBytes(1210, []byte{0xbf, 0x80, 0, 0})))},
...
// A headend advertisement includes TLV 1028 inside the Local Node
// Descriptors, even when its address differs from the BGP Router-ID.
// Dropping that sub-TLV used to leave the parent length unchanged and make
// forwarding the received MP_REACH fail during serialization.
func TestLsSrPolicyHeadendMPReach(t *testing.T) {
	wire, err := hex.DecodeString("900e005640044704ac1fff820000050049090000000000000000010000200200000400001a15020100040000000002040004ac1fff8204040004ac1eff82022a001803000000ac1fff640000000a000000000000000000000064")
	require.NoError(t, err)
	attr := &PathAttributeMpReachNLRI{}
	require.NoError(t, attr.DecodeFromBytes(wire))
	require.Len(t, attr.Value, 1)
	nlri := attr.Value[0].NLRI.(*LsAddrPrefix).NLRI.(*LsSrPolicyCandidatePathNLRI)
	headend := nlri.LocalNodeDesc.(*LsTLVNodeDescriptor).Extract()
	require.Equal(t, netip.MustParseAddr("172.30.255.130"), headend.LocalRouterID)
	require.Equal(t, netip.MustParseAddr("172.31.255.130"), headend.BGPRouterID)
	got, err := attr.Serialize()
	require.NoError(t, err)

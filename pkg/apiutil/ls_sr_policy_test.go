...
func Test_LsSrPolicyCandidatePathNLRIRoundTrip(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"NLRI { SRPOLICY_CP { LOCAL_NODE: {ASN: 65001, BGP LS ID: 0, OSPF AREA: 0, IGP ROUTER ID: , BGP ROUTER ID: 1.1.1.1, CONFEDERATION: 0, IPv4: 10.0.0.1, IPv6: invalid IP} ENDPOINT: 10.0.0.2 COLOR: 100 ORIGIN: 20 ORIGINATOR: 65001/1.1.1.1 DISCRIMINATOR: 1 SR:7 } }",
		},
		{
			"ipv6",
			&api.LsSrPolicyCandidatePathNLRI{
			...
			"NLRI { SRPOLICY_CP { LOCAL_NODE: {ASN: 65001, BGP LS ID: 0, OSPF AREA: 0, IGP ROUTER ID: 0000.0000.0001, BGP ROUTER ID: invalid IP, CONFEDERATION: 0, IPv4: invalid IP, IPv6: 2001:db8::1} ENDPOINT: 2001:db8::2 COLOR: 200 ORIGIN: 10 ORIGINATOR: 65002/2001:db8::1 DISCRIMINATOR: 2 SR:7 } }",
		},
	}
...
func Test_LsAttributeSrPolicyUnmarshalErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"constraints P and U flags", &api.LsAttributeSrPolicy{Constraints: &api.LsSrCandidatePathConstraints{Flags: &api.LsSrCandidatePathConstraintsFlags{ProtectedOnly: true, UnprotectedOnly: true}}}},
...
func TestLsSrPolicyHeadendValidation(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"wrong protocol", func(n *api.LsSrPolicyCandidatePathNLRI) {}, 2, true},
		{"missing Router-ID", func(n *api.LsSrPolicyCandidatePathNLRI) { n.LocalNode.LocalRouterIdIpv4 = "" }, 9, true},
		...
		{"headend missing ASN", func(n *api.LsSrPolicyCandidatePathNLRI) { n.LocalNode.Asn = 0 }, 9, true},
...
func TestLsSrSegmentValidation(t *testing.T) {
	for _, tt := range []struct {
		...
		{"mapped SID", &api.LsSrSegment{SegmentType: 2, Sid: "::ffff:10.0.0.1"}},
...
func TestLsSrBindingSIDValidation(t *testing.T) {
	for _, sid := range []string{"192.0.2.1", "::ffff:192.0.2.1", "fe80::1%eth0"} {
